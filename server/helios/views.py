...
from .security import (
    ...
    get_voter,
    user_can_admin_election,
    user_can_feature_election,
)
...
def castvote_shortcut(request, vote_tinyhash):
    ...
    return _castvote_shortcut_by_election(
        request, election_uuid=cast_vote.voter.election.uuid, cast_vote=cast_vote
    )
...
def elections_administered(request):
    if not can_create_election(request):
        return HttpResponseForbidden(
            "only an administrator has elections to administer"
        )
...
def election_new(request):
    ...
    if request.method == "GET":
        ...
        if election_form.is_valid():
            ...
            if (
                utils.urlencode(election_params["short_name"])
                == election_params["short_name"]
            ):
                election_params["uuid"] = str(uuid.uuid1())
                election_params["cast_url"] = settings.SECURE_URL_HOST + reverse(
                    one_election_cast, args=[election_params["uuid"]]
                )
                ...
                try:
                    ...
                    return HttpResponseRedirect(
                        settings.SECURE_URL_HOST
                        + reverse(one_election_view, args=[election.uuid])
                    )
    ...
    return render_template(
        request, "election_new", {"election_form": election_form, "error": error}
    )
...
def one_election_extend(request, election):
    if request.method == "GET":
        election_form = forms.ElectionTimeExtensionForm(
    ...
    return render_template(
        request,
        "election_extend",
        {"election_form": election_form, "election": election},
    )
...
def list_trustees_view(request, election):
    ...
    return render_template(
        request,
        "list_trustees",
        {"election": election, "trustees": trustees, "admin_p": admin_p},
    )
...
def new_trustee(request, election):
    if request.method == "GET":
        ...
        trustee.save()
        return HttpResponseRedirect(
            settings.SECURE_URL_HOST + reverse(list_trustees_view, args=[election.uuid])
        )
    ...
    if election:
        ...
        if trustee:
            if trustee.secret == trustee_secret:
                ...
                return HttpResponseRedirect(
                    settings.SECURE_URL_HOST
                    + reverse(trustee_home, args=[election.uuid, trustee.uuid])
                )
...
def trustee_send_url(request, election, trustee_uuid):
    ...
    url = settings.SECURE_URL_HOST + reverse(
...
""" % (
    ...
    utils.send_email(
        ...
        ["%s <%s>" % (trustee.name, trustee.email)],
        "your trustee homepage for %s" % election.name,
        body,
    )
...
def trustee_home(request, election, trustee):
    return render_template(
        request, "trustee_home", {"election": election, "trustee": trustee}
    )
...
def trustee_check_sk(request, election, trustee):
    return render_template(
        request, "trustee_check_sk", {"election": election, "trustee": trustee}
    )
...
def trustee_upload_pk(request, election, trustee):
    if request.method == "POST":
        ...
        # verify the pok
        if not trustee.public_key.verify_sk_proof(
            trustee.pok, algs.DLog_challenge_generator
        ):
            raise Exception("bad pok for this public key")

        trustee.public_key_hash = cryptoutils.hash_b64(
        ...
        try:
            election.admin.send_message(
                "%s - trustee pk upload" % election.name,
                "trustee %s (%s) uploaded a pk." % (trustee.name, trustee.email),
            )
        except:
...
def one_election_cast(request, election):
  ...
  """
    ...
    return HttpResponseRedirect(
        ...
        % (
            settings.SECURE_URL_HOST,
            reverse(one_election_cast_confirm, args=[election.uuid]),
        )
    )
    ...
    if password_login_form.is_valid():
        try:
            ...
            redirect_url = (
                ...
                + "?"
                + urllib.parse.urlencode(
                    {"bad_voter_login": "1", "return_url": return_url}
                )
            )
        ...
        redirect_url = (
            login_url
            + "?"
            + urllib.parse.urlencode({"bad_voter_login": "1", "return_url": return_url})
        )
...
def one_election_cast_confirm(request, election):
    ...
    if ("encrypted_vote" not in request.session) or request.session[
        "encrypted_vote"
    ] == None:
    ...
    if voter:
        vote = datatypes.LDObject.fromDict(
            utils.from_json(encrypted_vote), type_hint="legacy/EncryptedVote"
        ).wrapped_obj
    ...
    if request.method == "GET":
        ...
        if voter and voter.can_update_status():
            status_update_label = (
                voter.user.update_status_template() % "your smart ballot tracker"
            )
            status_update_message = (
                "I voted in %s - my smart tracker is %s.. #heliosvoting"
    ...
    if request.method == "POST":
        ...
        tasks.cast_vote_verify_and_store.delay(
            cast_vote_id=cast_vote.id, status_update_message=status_update_message
        )
        ...
        return HttpResponseRedirect(
            "%s%s"
            % (settings.URL_HOST, reverse(one_election_cast_done, args=[election.uuid]))
        )
...
def one_election_cast_done(request, election):
  ...
  """
    ...
    return render_template(
        request,
        "cast_done",
        {"election": election, "vote_hash": vote_hash, "logout": logout},
...
def one_election_audited_ballots(request, election):
  ...
  """
    ...
    audited_ballots = AuditedBallot.get_by_election(
        election, after=after, limit=limit + 1
    )
...
""" % (
            ...
            # log it
            election.append_log(
                "Voter %s/%s and their vote were removed after election frozen"
                % (voter.voter_type, voter.voter_id)
            )
...
def one_election_copy(request, election):
    ...
    return HttpResponseRedirect(
        settings.SECURE_URL_HOST + reverse(one_election_view, args=[new_election.uuid])
    )
...
def one_election_compute_tally(request, election):
  ...
  """
    ...
    if request.method == "GET":
        return render_template(
            request, "election_compute_tally", {"election": election}
        )
...
def trustee_decrypt_and_prove(request, election, trustee):
    ...
    return render_template(
        request, "trustee_decrypt_and_prove", {"election": election, "trustee": trustee}
    )
...
def trustee_upload_decryption(request, election, trustee_uuid):
    ...
    trustee.decryption_factors = [
        [
            datatypes.LDObject.fromDict(factor, type_hint="core/BigInteger").wrapped_obj
            for factor in one_q_factors
        ]
        for one_q_factors in factors_and_proofs["decryption_factors"]
    ]

    # each proof needs to be deserialized
    trustee.decryption_proofs = [
        [
            datatypes.LDObject.fromDict(proof, type_hint="legacy/EGZKProof").wrapped_obj
            for proof in one_q_proofs
        ]
        for one_q_proofs in factors_and_proofs["decryption_proofs"]
    ]
    ...
    if trustee.verify_decryption_proofs():
        ...
        try:
            # send a note to admin
            election.admin.send_message(
                "%s - trustee partial decryption" % election.name,
                "trustee %s (%s) did their partial decryption."
    ...
    if request.method == "POST":
        ...
        if request.POST.get("send_email", ""):
            return HttpResponseRedirect(
                ...
                % (
                    settings.SECURE_URL_HOST
                    + reverse(voters_email, args=[election.uuid]),
  ...
  """
    ...
    if request.method == "GET":
        return render_template(
            request,
            "voters_upload",
            {"election": election, "error": request.GET.get("e", None)},
        )
    ...
    if request.method == "POST":
        if bool(request.POST.get("confirm_p", 0)):
            ...
            return HttpResponseRedirect(
                settings.SECURE_URL_HOST
                + reverse(voters_list_pretty, args=[election.uuid])
            )
        else:
            # we need to confirm
            if "voters_file" in request.FILES:
                ...
                if False in [validate_email(v["email"]) for v in voters]:
                    problems.append(
                        "those don't look like correct email addresses. Are you sure you uploaded a file with email address as second field?"
                    )
                ...
                return HttpResponseRedirect(
                    ...
                    % (
                        ...
                        urllib.parse.urlencode(
                            {"e": "no voter file specified, try again"}
                        ),
                    )
                )
...
def voters_email(request, election):
    ...
    default_subject = render_template_raw(
        None, "email/%s_subject.txt" % template, {"custom_subject": "&lt;SUBJECT&gt;"}
    )
    default_body = render_template_raw(
        None,
    ...
    if request.method == "GET":
        email_form = forms.EmailVotersForm(
            initial={"subject": election.name, "body": " "}
        )
        if voter:
